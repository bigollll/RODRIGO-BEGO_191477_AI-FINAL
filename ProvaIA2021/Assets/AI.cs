...
public class AI : MonoBehaviour
{
    ...
    float speed1 = 10000f;           //velocidade para variação de fuga
    float speed2 = 20f;              //velocidade para variação dpois da fuga
    ...
    public void PickRandomDestination()                                                       //metodo que da um destino aleatorio.
    {
        ...
        agent.speed = speed2;                                                                 //volta a velocidade para 20.
    ...
    [Task]
    public void LookAtTargetFuga()                                                                                                              //metodo para detectar o target.
    {
        Vector3 direction = target - this.transform.position;                                                                                   //a direção do vertor é o targt.
        this.transform.rotation = Quaternion.Slerp(this.transform.rotation, Quaternion.LookRotation(direction), Time.deltaTime * rotSpeed);     //rotação do ai para virar para o player.
        ...
        {
            Task.current.debugInfo = string.Format("anfle={0}", Vector3.Angle(this.transform.forward, direction));                              //ativa a task no ai.
            agent.speed = speed1;                                                                                                               //muda o speed do agent para 10.0000
        }
    ...
    public void EmpurrarPraFora()
    {
        ...
        {
            transform.position = Vector3.MoveTowards(transform.position, player.position, speed1 * Time.deltaTime);                      //vai aé o player em uma velocidade alta e o empurra
        }
    ...
    public void Congela()
    {
        Vector3 direction = player.position;                                                                           //a direção do vertor é o targt.
        this.transform.rotation = Quaternion.Slerp(this.transform.rotation, Quaternion.LookRotation(direction), Time.deltaTime * rotSpeed);     //rotação do ai para virar para o player.
